...
MurataReceiver(port: int, buffer_size: int = 1024,
               ...
               unparsed_callback: Optional[Callable] = None)
```
...
- `unparsed_callback` (callable, optional): 未対応センサーや解析不能データ受信時に呼ばれるコールバック。`(unparsed_data: dict, addr: tuple) -> None`
...
`unparsed_callback` は、UDPで受信したデータが未対応センサー、非村田形式、チェックサム不正などで解析済みセンサーデータにできなかった場合に呼ばれます。UDPは同じデータを再受信できないため、利用アプリ側で保存や後日の再解析を行う用途に使用します。

```python
def on_unparsed(unparsed_data, addr):
    print(unparsed_data["reason"])
    print(unparsed_data["sensor_type_code"])
    save_raw_packet(unparsed_data["raw_data"])
```
...
**パラメータ:** `port`, `buffer_size`, `logger`（いずれも MurataReceiver と同様）、`include_unparsed`（Trueの場合、未解析データもイテレーションで返す）
...
`start()` 呼び出し後に `async for sensor_data, addr in receiver:` で受信データをイテレートできる。解析済みデータの `sensor_data` は辞書（`sensor_type`, `sensor_type_code`, `timestamp`, `values`, `info`, `addr`）。

`include_unparsed=True` の場合、未知センサーなどの未解析データも同じイテレーションで返す。未解析データは `parsed: False`, `raw_data`, `reason`, `sensor_type_code` などを持つ。
...
**パラメータ:**
- `line` (str): テキスト行
- `strict` (bool): Trueの場合は従来通り解析不能時に例外を送出する。Falseの場合は未解析結果を辞書で返す。
...
def build_unparsed_data(
    raw_data: bytes,
    addr: Tuple[str | None, int | None] = (None, None),
...
**説明:**
解析できなかった受信データを、`unparsed_callback` や `parse_text_line(..., strict=False)` と同じ形式の辞書に変換します。独自の受信処理を持つアプリで、未解析データの保存形式をそろえたい場合に使用します。
...
(
    {
        "sensor_type": "temperature_and_humidity",
        "type_codes": ("030301FF",),
        "description": "温湿度センサー",
        "products": ("1AN",),
    },
...
**説明:**
対応済みセンサータイプ名だけの一覧を返します。センサーコードや説明が不要で、`sensor_type` の選択肢や分岐条件だけを扱いたい場合に使用します。
...
**例:**
```python
from murata_sensor import get_supported_sensor_types

if "vibration" in get_supported_sensor_types():
    print("振動センサーに対応しています")
```
...
**説明:**
指定したセンサータイプ名に対応しているかを返します。解析結果の `sensor_type` や設定ファイル上のセンサータイプを検証したい場合に使用します。

**例:**
```python
from murata_sensor import is_supported_sensor_type

if is_supported_sensor_type("vibration"):
    print("指定されたセンサータイプに対応しています")
```
...
**説明:**
指定した8桁のセンサーコードに対応しているかを返します。受信前の設定値や、電文から取り出した `0303[tt][SS]` 形式のコードを検証したい場合に使用します。大文字・小文字の違いは吸収します。

**例:**
```python
from murata_sensor import is_supported_sensor_code

if is_supported_sensor_code("03030900"):
    print("指定されたセンサーコードに対応しています")
```
