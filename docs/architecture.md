...
- 複数のセンサータイプに対応したデータ処理
- センサーデータの履歴管理と最新データの提供

### 対応センサータイプ
- 温湿度、振動、電流・電圧、熱電対、漏水、防水中継機などのセンサーに対応
- 対応済みセンサー一覧は `get_supported_sensors()` で取得可能
- センサーコードの正は `SENSOR_TYPE`、表示用メタデータは対応センサー一覧APIで提供
- `parse_verified` が True のタイプは実電文による主要値の自動テスト済み。False は登録済みだが値解析の自動検証が未整備
...
- **murata_sensor.py**: センサーデータの解析処理
  ...
  - 対応済みセンサー一覧API（`get_supported_sensors()` など）
